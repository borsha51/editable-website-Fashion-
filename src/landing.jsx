...
export default function Landing(){
  ...
  return <div className="landing">
    ...
    <main>
      ...
      {show('story')&&<section id="story" className="lstory"><div className="storyImage"><img src={(data.products?.[0]?.image||site.products[0].image)} alt={site.products[0].name}/><small>SCULPTED LINEN BLAZER / 01</small></div><div className="storyCopy"><span>OUR APPROACH</span><h2>Fewer, better<br/><i>things.</i></h2><p>From a precise shoulder to a soft hand-feel, the details are intentionally quiet. We design around repeat wear, not one-off moments.</p><a href="/" className="ltext">Discover Nexora <ArrowRight/></a></div></section>
      <section id="journal" className="ljournal"><div><span>THE JOURNAL</span><h2>Style notes<br/><i>without the noise.</i></h2></div>{(L.journal||[]).map((j,i)=><article key={i}><small>{j.category||'STYLE NOTE'}</small><h3>{j.title}</h3><p>{j.text}</p><a href={j.link||'/'}>{j.link?'Read the edit':'Read the note'} <ArrowRight size={15}/></a></article>)}</section>}
      {show('reviews')&&<section className="admin-review-section"><span>CLIENT NOTES</span><h2>Real words,<br/><i>real style.</i></h2><div className="review-strip">{(L.testimonials||[]).map(r=><article key={r.id}><img src={r.image||site.products[0].image} alt={r.name}/><b>{r.name}</b><small>{'★'.repeat(Number(r.rating||5))}</small><p>“{r.review}”</p></article>)}</div></section>{show('newsletter')&&<section className="lcta newsletter"><span>NEXORA WEB</span><h2>{L.newsletter?.heading||'Make space for your style.'}</h2><p>{L.newsletter?.text}</p><form onSubmit={async e=>{e.preventDefault();const r=await fetch('/api/subscribe',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email})});setNotice(r.ok?'You’re on the list.':'Please enter a valid email.');if(r.ok)setEmail('')}}><input type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="Your email address"/><button className="lbutton dark">{L.newsletter?.button||'Join the list'} <ArrowRight/></button></form><small>{notice}</small></section>}
    </main>

...
export default function Landing(){
  ...
  const L=data.landing||{};const S=L.sections||{};const show=id=>S[id]!==false;const order=id=>(L.sectionOrder||['hero','intro','collections','story','journal','reviews','newsletter']).indexOf(id);
  ...
  return <div className="landing">
    ...
    <main>
      ...
      {show("intro")&&<section id="collection" className="lintro" style={{order:order("intro")}}><span>{L.intro?.eyebrow||'THE NEXORA EDIT'}</span><h2>{L.intro?.title||'Designed for now. Made for longer.'}</h2><p>{L.intro?.text||'We believe personal style is built slowly: through proportion, texture and the confidence to repeat what works.'}</p></section>}
      {show('collections')&&<section className="ltiles" style={{order:order("collections")}}>
      ...
      {show('story')&&<section id="story" className="lstory" style={{order:order("story")}}><div className="storyImage"><img src={L.story?.image||data.products?.[0]?.image||site.products[0].image} alt={L.story?.heading||'Nexora story'}/><small>NEXORA / STORY</small></div><div className="storyCopy"><span>{L.story?.eyebrow||'OUR APPROACH'}</span><h2>{L.story?.heading||'Fewer, better things.'}</h2><p>{L.story?.text||''}</p><div className="stats">{(L.story?.stats||[]).map((s,i)=><div key={i}><b>{s[0]}</b><small>{s[1]}</small></div>)}</div></div></section>}
{show('journal')&&<section id="journal" className="ljournal" style={{order:order("journal")}}><div><span>THE JOURNAL</span><h2>Style notes<br/><i>without the noise.</i></h2></div>{(L.journal||[]).map((j,i)=><article key={i}>{j.image&&<img src={j.image} alt=""/>}<small>{j.category||'STYLE NOTE'} {j.date&&'· '+j.date}</small><h3>{j.title}</h3><p>{j.text}</p><a href={j.link||'/'}>Read the edit <ArrowRight size={15}/></a></article>)}</section>}
{show('reviews')&&<section className="admin-review-section" style={{order:order("reviews")}}><span>CLIENT NOTES</span><h2>Real words,<br/><i>real style.</i></h2><div className="review-strip">{(L.testimonials||[]).map(r=><article key={r.id}><img src={r.image||site.products[0].image} alt={r.name}/><b>{r.name}</b><small>{'★'.repeat(Number(r.rating||5))}</small><p>“{r.review}”</p></article>)}</div></section>}
{show('newsletter')&&<section className="lcta newsletter" style={{order:order("newsletter")}}><span>NEXORA WEB</span><h2>{L.newsletter?.heading||'Make space for your style.'}</h2><p>{L.newsletter?.text}</p><form onSubmit={async e=>{e.preventDefault();const r=await fetch('/api/subscribe',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email})});setNotice(r.ok?'You’re on the list.':'Please enter a valid email.');if(r.ok)setEmail('')}}><input type="email" required value={email} onChange={e=>setEmail(e.target.value)} placeholder="Your email address"/><button className="lbutton dark">{L.newsletter?.button||'Join the list'} <ArrowRight/></button></form><small>{notice}</small></section>}</main>

  ...
  return <div className="landing">
    ...
    <main>
      <section className="lhero">
        <img src={L.hero?.image||(L.collections?.[0]?.image||site.collections[0].image)} alt="Nexora editorial fashion"/>
      ...
      <section className="ltiles">
        ...
        <a href="/" className="ltile"><img src={(L.collections?.[1]?.image||site.collections[1].image)} alt="After hours"/><div><span>02 / AFTER HOURS</span><h3>Sharper after dark</h3><b>Explore <ArrowRight/></b></div></a>
      </section>
      ...
      <section className="admin-review-section"><span>CLIENT NOTES</span><h2>Real words,<br/><i>real style.</i></h2><div className="review-strip">{(L.testimonials||[]).map(r=><article key={r.id}><img src={r.image||site.products[0].image} alt={r.name}/><b>{r.name}</b><small>{'★'.repeat(Number(r.rating||5))}</small><p>“{r.review}”</p></article>)}</div></section><section className="lcta"><span>NEXORA WEB</span><h2>Make space for<br/><i>your style.</i></h2><a className="lbutton dark" href="/">Shop now <ArrowRight/></a></section>
    </main>
